...
function salesReportValue(demoInput) {
  return [
    BigInt(demoInput.loan.id),
    BigInt(demoInput.merchant.id),
    BigInt(demoInput.report.dayIndex),
    BigInt(demoInput.report.grossSales),
    BigInt(demoInput.report.nonce)
  ];
}
...
function buildManifest(overrides) {
  ...
  const encodedSalesReport = encodeAbiParameters(salesReportAbi, salesReportValue(demoInput));
...
export { buildManifest, buildTranscript, competitorSignalFromSales, demo, repaymentFor, salesReportAbi, withDemoOverrides };
